...
/**
 * 全局只读模式只允许 URL 参数提供 API Key，其他配置均由部署端控制。
 */
function buildReadOnlySettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  ...
  if (importedSettings && typeof importedSettings === 'object' && !Array.isArray(importedSettings)) {
    ...
    if (Array.isArray(profiles)) {
      ...
      if (typeof matched?.apiKey === 'string') apiKey = matched.apiKey
    }
  }

  const apiKeyParam = searchParams.get('apiKey')
  if (apiKeyParam !== null) apiKey = apiKeyParam.trim()
...
export function buildSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  if (isApiKeyPromptRequired()) return {}
  if (isApiConfigReadOnly()) return buildReadOnlySettingsFromUrlParams(currentSettings, searchParams)
